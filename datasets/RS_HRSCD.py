...
# MEAN_A = np.array([111.53, 113.11, 87.54])
# STD_A  = np.array([55.35, 41.48, 39.27])
...
#         img_A = io.imread(self.imgs_list_A[idx]).astype(np.float32)
#         img_A = normalize_image(img_A, 'A')
#         img_B = io.imread(self.imgs_list_B[idx]).astype(np.float32)
...
#         return F.to_tensor(img_A), F.to_tensor(img_B), torch.from_numpy(label_A).squeeze(), torch.from_numpy(label_B).squeeze()
...
#     def __getitem__(self, idx):
#         img_A = self.imgs_A[idx].astype(np.float32)
#         img_B = self.imgs_B[idx].astype(np.float32)
#         img_A = normalize_image(img_A, 'A')
#         img_B = normalize_image(img_B, 'B')
#         return F.to_tensor(img_A), F.to_tensor(img_B)
...
ST_COLORMAP = [
    [255, 255, 255],
    [255, 0, 0],  # artificial surfaces
    [255, 255, 0], # agricultural areas
    [0, 255, 0], # forests
    [0, 255, 255], # wetlands
    [0, 0, 255], # water
]
...
MEAN_B = np.array([105.9, 119.79, 97.56])
STD_B = np.array([49.87, 40.02, 35.18])
