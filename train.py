...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    while True:
        ...
        if acc_meter.avg>bestaccT: bestaccT=acc_meter.avg # type: ignore
        if Fscd_v>bestFscdV:
            bestFscdV=Fscd_v
            bestaccV=acc_v
            bestloss=loss_v
        ...
        print('Total time: %.1fs Best rec: Train acc %.2f, Val Fscd %.2f acc %.2f loss %.4f' %(time.time()-begin_time, bestaccT*100, bestFscdV*100, bestaccV*100, bestloss)) # type: ignore
